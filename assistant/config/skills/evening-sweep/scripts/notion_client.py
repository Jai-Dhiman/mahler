...
def _extract_task(page: dict) -> dict:
    ...
    return {"id": page["id"], "title": title, "status": status, "due": due, "priority": priority, "last_edited_time": last_edited_time}
...
class NotionClient(_NotionBase):
    ...
    def list_tasks(
        ...
        if last_edited_after is not None:
            filters.append({"timestamp": "last_edited_time", "last_edited_time": {"on_or_after": last_edited_after}})
